...
# Isolation Forest + multi-escala fixo [(96,48), (64,32), (48,24)]
# ---------------------------------------------------------------
...
NU          = 0.10  # Contamination similar to nu in OC-SVM
RANDOM_STATE = 42   # For reproducibility

GABOR_THETAS = [0, np.pi/6, np.pi/3, np.pi/2, 2*np.pi/3, 5*np.pi/6]
GABOR_FREQS  = [0.05, 0.10, 0.20, 0.30]

# Pós-processamento (ajustado para mais sensibilidade)
THR_PERC    = 75        # percentil do heat (baixado para detectar mais)
...
# ======================== Features ==============================
def radial_power_spectrum(img_gray, nbins=32):  # Aumentado para 32 bins
    h, w = img_gray.shape
...
def feats_for_patch(img_rgb):
    ...
    # GLCM features
    glcm = graycomatrix(g_u8, distances=[1], angles=[0, np.pi/4, np.pi/2, 3*np.pi/4], levels=256, symmetric=True, normed=True)
    glcm_feats = []
    for prop in ['contrast', 'dissimilarity', 'homogeneity', 'energy', 'correlation']:
        glcm_feats += list(graycoprops(glcm, prop).ravel())

    rps = radial_power_spectrum(g_norm, nbins=32)
    # Stats expandidas com skew e kurtosis
    patch_stats = [g_norm.mean(), g_norm.std(), np.median(g_norm), stats.skew(g_norm.ravel()), stats.kurtosis(g_norm.ravel())]
    patch_stats = np.array(patch_stats, dtype=np.float32)

    return np.hstack([np.float32(gabor_feats), np.float32(hist_lbp), np.float32(glcm_feats), rps, patch_stats])

def extract_patches(img_rgb, patch_size, stride):
    H, W, _ = img_rgb.shape
    if H < patch_size or W < patch_size:
        # Resize para caber pelo menos um patch (melhoria para imagens pequenas)
        scale = patch_size / min(H, W)
        newH, newW = int(H * scale), int(W * scale)
        img_rgb = cv2.resize(img_rgb, (newW, newH))
        H, W = newH, newW
        if VERBOSE:
            print(f"[INFO] Redimensionado para ({newH}, {newW}) para patch={patch_size}")
...
# ======================== Treino ================================
def load_training_patches_from_dir(img_dir: Path, scales):
    ...
    for p in files:
        ...
        if bgr is None:
            print(f"[AVISO] não li {p}"); continue
        rgb = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
        feats_all = []
        for ps, st in scales:
            patches, _, _ = extract_patches(rgb, ps, st)
            if len(patches) == 0: continue
            feats = [feats_for_patch(px) for px in patches]
            feats_all.extend(feats)
            if VERBOSE:
                print(f"[VERBOSE] {p.name}: {len(patches)} patches na escala ({ps},{st})")
        if feats_all:
            X_feats.append(np.vstack(feats_all))
    return np.vstack(X_feats) if X_feats else None

def train_model(scales):
    X = None; trained_on = None
    if DIR_PADRAO.exists():
        X = load_training_patches_from_dir(DIR_PADRAO, scales)
        if X is not None: trained_on = "PADRÃO (normais)"
    if X is None and DIR_FALHAS.exists():
        X = load_training_patches_from_dir(DIR_FALHAS, scales)
    ...
    # Isolation Forest com contamination similar ao nu
    iso_forest = IsolationForest(n_estimators=100, contamination=NU, random_state=RANDOM_STATE).fit(scaler.transform(X))
    
    return scaler, iso_forest, trained_on

# ======================== Scoring ===============================
def score_grid(scaler, iso_forest, rgb, trained_on, patch_size, stride):
    ...
    # Isolation Forest: decision_function retorna scores negativos para anomalias
    d = iso_forest.decision_function(scaler.transform(X)).ravel()
    
    # Inverter se treinado em padrões (anomalias têm scores negativos)
    # Se treinado em falhas, anomalias (padrões) têm scores negativos, mas queremos detectar falhas como normais? Ajuste conforme lógica.
    # Assumindo treino em padrões: scores baixos = anomalias (falhas)
...
def analyze_single_scale(scaler, iso_forest, rgb, trained_on, patch_size, stride):
    grid = score_grid(scaler, iso_forest, rgb, trained_on, patch_size, stride)
    if grid is None: return None
    return spread_heat_from_grid(grid, rgb.shape, patch_size, stride)

def analyze_multi_scale(scaler, iso_forest, rgb, trained_on, scales):
    H, W = rgb.shape[:2]
    accum = np.zeros((H, W), dtype=np.float32)
    any_scale = False
    for ps, st in scales:
        heat = analyze_single_scale(scaler, iso_forest, rgb, trained_on, ps, st)
...
def run():
    ...
    # Treino usando todas as escalas
    scaler, iso_forest, trained_on = train_model(SCALES)
    ...
    for p in analis:
        ...
        heat = analyze_multi_scale(scaler, iso_forest, rgb, trained_on, SCALES)
        if heat is None:
