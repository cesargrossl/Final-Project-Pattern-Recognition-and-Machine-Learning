...
# OC-SVM + multi-escala [(96,48), (64,32)]
# ---------------------------------------------------------------
...
# ======================== Hiperparâmetros =======================
SCALES      = [(96,48), (64,32)]   # reduzido para acelerar

NU          = 0.10
GAMMA       = "scale"

# Reduzido: 4 orientações, 3 frequências
GABOR_THETAS = [0, np.pi/4, np.pi/2, 3*np.pi/4]
GABOR_FREQS  = [0.05, 0.15, 0.30]

# Pós-processamento
THR_PERC    = 75
MIN_BOX     = 0.02
NMS_IOU     = 0.25
VERBOSE     = False  # menos prints = mais rápido
...
# ======================== Features ==============================
def radial_power_spectrum(img_gray, nbins=16):  # reduzido para acelerar
    h, w = img_gray.shape
...
def feats_for_patch(img_rgb):
    ...
    patch_stats = [g_norm.mean(), g_norm.std(), np.median(g_norm),
                   stats.skew(g_norm.ravel()), stats.kurtosis(g_norm.ravel())]
    patch_stats = np.array(patch_stats, dtype=np.float32)

    return np.hstack([np.float32(gabor_feats), np.float32(hist_lbp),
                      np.float32(glcm_feats), rps, patch_stats])
    ...
    for p in files:
        ...
        for ps, st in scales:
            ...
            feats = Parallel(n_jobs=-1, prefer="threads")(
                delayed(feats_for_patch)(px) for px in patches
            )
...
def score_grid(scaler, oc, rgb, trained_on, patch_size, stride):
    ...
    X = Parallel(n_jobs=-1, prefer="threads")(
        delayed(feats_for_patch)(px) for px in patches
    )
    d = oc.decision_function(scaler.transform(np.vstack(X))).ravel()
...
def run():
    ...
    print(f"[INFO] Multi-escala -> {SCALES} | thr={THR_PERC} | min_box={MIN_BOX}")
