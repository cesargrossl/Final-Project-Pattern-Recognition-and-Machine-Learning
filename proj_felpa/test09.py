...
# OC-SVM + multi-escala [(96,48), (64,32)] com gate de confiança
...
# ======================== Hiperparâmetros =======================
SCALES      = [(96,48), (64,32)]   # (patch, stride)
PS_TRAIN    = max(ps for ps, _ in SCALES)
ST_TRAIN    = PS_TRAIN // 2

NU          = 0.05                 # mais conservador
GAMMA       = "scale"

GABOR_THETAS = [0, np.pi/6, np.pi/3, np.pi/2, 2*np.pi/3, 5*np.pi/6]
GABOR_FREQS  = [0.05, 0.10, 0.20, 0.30]

THR_PERC    = 90                   # percentil do heat
MIN_BOX     = 0.03                 # proporção mínima de área
NMS_IOU     = 0.25
CONF_MIN    = 0.60                 # confiança mínima absoluta do heat
VERBOSE     = True
...
def boxes_from_pixel_heat(heat, min_box_ratio=MIN_BOX, thr_perc=THR_PERC, nms_iou=NMS_IOU, conf_min=CONF_MIN):
    H, W = heat.shape[:2]

    # Gate de confiança: se nem o pico atinge o mínimo, não marca nada
    if float(heat.max()) < conf_min:
        mask = np.zeros((H, W), dtype=np.uint8)
        return [], [], mask

    # Limiar final = max(percentil, confiança mínima)
    thr_p = np.percentile(heat, thr_perc)
    thr = max(thr_p, conf_min)
...
def run():
    ...
    # Treino usando o maior patch da lista de escalas (estável)
    ...
    print(f"[INFO] Multi-escala FIXO -> {SCALES} | thr={THR_PERC} | min_box={MIN_BOX} | conf_min={CONF_MIN}")
    ...
    for p in analis:
        ...
        boxes, scores, mask = boxes_from_pixel_heat(
            heat, min_box_ratio=MIN_BOX, thr_perc=THR_PERC, nms_iou=NMS_IOU, conf_min=CONF_MIN
        )
